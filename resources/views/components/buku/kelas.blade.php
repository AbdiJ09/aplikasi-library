<div class="w-full p-1 shadow-xl bg-zinc-200 rounded-xl relative my-3">
    <div class="grid grid-cols-5 content-center h-10 justify-items-center">
        <h4 class="text-xs text-neutral-700 absolute left-4 top-2/4 -translate-y-2/4">
            {{ $item->buku->kode_buku }}</h4>
        <span></span>
        <h4 class="text-xs text-neutral-700 my-auto">{{ Str::limit($item->buku->judul, 10) }}</h4>
        <h4 class="text-xs text-neutral-700 my-auto">{{ $item->kelas->nama }}</h4>
        <h4 class="text-xs text-neutral-700 my-auto">{{ $item->jurusan->nama }}</h4>
            ...
            <div class="relative w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 text-center">
                        ...
                        <form action="{{ route('buku.destroy', $item->id) }}" class="inline-block" method="post">
