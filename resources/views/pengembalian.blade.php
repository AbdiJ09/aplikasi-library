...
@section('detailBuku')
            ...
            <div class="absolute right-5">
                <div class="input-container">
                    <form action="/pengembalian">
                        <input placeholder="Search..." class="inputSearch pengembalianSearch" name="search" type="text">
                    </form>
    ...
    <div
        class ="w-full mx-auto z-[20]  relative  my-40 lg:my-52 md:grid md:grid-cols-2 md:gap-6 lg:grid {{ $pengembalian->count() ? 'lg:grid-cols-3' : 'lg:grid-cols-1' }} lg:gap-6 pengembalianContainer bg-gradient-to-r from-purple-950 via-black to-purple-950 py-5 px-1 lg:p-8 rounded-xl ">
        @if ($pengembalian->count())
            <form action="{{ route('pengembalian') }}" id="formPengembalian"
                ...
                <select
                    ...
                    <option value="dikembalikan" {{ request('fillter') == 'dikembalikan' ? 'selected' : '' }}
                        class="text-xs lg:text-lg">Dikembalikan</option>
                    <option value="belum-dikembalikan" {{ request('fillter') == 'belum-dikembalikan' ? 'selected' : '' }}
                        class="text-xs lg:text-lg">Belum
                        dikembalikan</option>
                    <option value="telat" {{ request('fillter') == 'telat' ? 'selected' : '' }} class="text-xs lg:text-lg">
                        Telat</option>
                    <option value="aman" {{ request('fillter') == 'aman' ? 'selected' : '' }} class="text-xs lg:text-lg">
                        Aman</option>
                </select>
            </form>
            @if (request('fillter') == 'belum-dikembalikan')
                @foreach ($tidakDikembalikan as $item)
                    <div
                        ...
                        <div class="flex justify-between items-center space-x-3">
                            <div class="flex flex-col justify-center items-center space-y-2">
                                ...
                                @endif
                                <span
                                    class=" text-black font-medium uppercase text-[8px] text-center md:text-sm lg:text-xs bg-error px-1 rounded-xl">Belum
                                    dikembalikan</span>
                            </div>
                            <div class="w-full">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div class="bg-transparent border rounded-lg border-gray-700 p-1 text-gray-400 w-5/6">
                                        <p class="text-xs md:text-sm lg:text-sm">Balikin bukunya ganten/cantik</p>
                                    </div>
                                </div>
                            ...
                            <div
                                ...
                                <p class="text-black text-[8px] lg:text-[10px] mt-[4.5rem]   ms-5 font-bold flex flex-col">
                                    Peminjaman<span class="text-lg lg:text-2xl ms-4 -mt-1">{{ $item->lama_pinjam }}H</span>
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            @endif
        @else
            <h1 class="text-center text-white font-bold text-2xl uppercase">Pengembalian belum ada</h1>
        @endif

        @vite('resources/js/pengembalianSearch.js')
    @endsection
